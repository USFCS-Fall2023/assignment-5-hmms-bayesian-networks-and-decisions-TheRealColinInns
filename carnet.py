...
cpd_keys = TabularCPD(
    variable="KeyPresent", variable_card=2, values=[[0.99], [0.01]],
...
car_model.add_cpds( cpd_starts, cpd_ignition, cpd_gas, cpd_radio, cpd_battery, cpd_moves, cpd_keys)
...
print(car_infer.query(variables=["Radio", "Gas"], evidence={"Battery": "Works"}))
print(car_infer.query(variables=["Ignition", "Gas"], evidence={"Moves": "no"}))
print(car_infer.query(variables=["Starts"], evidence={"Radio": "turns on", "Gas": "Full"}))
print(car_infer.query(variables=["KeyPresent"], evidence={"Radio": "turns on", "Gas": "Full", "Starts": "no"}))
